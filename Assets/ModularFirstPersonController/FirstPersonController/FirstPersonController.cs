...
using Mirror;  // Ajout pour le multijoueur
...
public class FirstPersonController : NetworkBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Si ce n'est pas le joueur local, désactive la caméra pour éviter les conflits
        if (!isLocalPlayer)
        {
        ...
        // Initialisation pour le joueur local
        playerCamera.fieldOfView = fov;
    }

    void Start()
    {
        // Ne s'exécute que pour le joueur local
        if (!isLocalPlayer)
            return;
    ...
    // Vérifie si le joueur est au sol via un raycast
    ...
    {
        // Activer la caméra seulement pour le joueur local
        if (playerCamera != null)
        {
